...
export default function AreaPage({ kind = 'area' }: { kind?: 'area' | 'micro-area' }) {
  ...
  const discoveryLinks = [
    {
      eyebrow: entry.name,
      title: `Private chef fine dining in ${entry.name}`,
      description: 'Chef-led villa dinners, tasting menus, and romantic evenings planned around your stay.',
      href: '/fine-dining/private-chef-bali',
    },
    {
      eyebrow: entry.name,
      title: `Villa catering in ${entry.name}`,
      description: 'Compare breakfast, lunch, dinner, and chef-led group menus for villa stays of any length.',
      href: '/catering/villa-catering',
    },
    {
      eyebrow: 'Celebrations',
      title: `Events and celebrations in ${entry.name}`,
      description: 'Useful for weddings, birthdays, and larger guest counts that need staffing and service flow.',
      href: '/events',
    },
    {
      eyebrow: 'Booking',
      title: `Book a private chef in ${entry.name}`,
      description: 'Move straight into a quote once you know your villa area, dates, and guest count.',
      href: '/book',
    },
  ]
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF8] text-[#1A1A1A]">
      ...
      <StrategicLinksSection
        ...
        title={`Plan the right service in ${entry.name}`}
        description={`Use these pages to compare fine dining, villa catering, celebrations, and the quickest booking route for your stay in ${entry.name}.`}
        cards={discoveryLinks}
      />
