...
export default function LocationLandingPage({ config }: { config: LocationLandingPageConfig }) {
  ...
  const planningLinks = [
    {
      eyebrow: config.name,
      title: `Private chef fine dining in ${config.name}`,
      description: config.serviceAvailability[0]?.summary ?? 'Compare chef-led villa dinners and private dining options for your stay.',
      href: config.serviceAvailability[0]?.href ?? '/fine-dining',
    },
    {
      eyebrow: config.name,
      title: `Villa catering in ${config.name}`,
      description: config.serviceAvailability[1]?.summary ?? 'Explore larger-format catering for group stays and villa celebrations.',
      href: config.serviceAvailability[1]?.href ?? '/catering',
    },
    {
      eyebrow: 'Booking',
      title: `Book myCHEF in ${config.name}`,
      description: 'Share your villa, dates, and guest count to move directly from research into a confirmed quote.',
      href: '/book',
    },
    {
      eyebrow: 'Contact',
      title: 'Speak to the myCHEF concierge',
      description: 'Use our contact page if you want menu guidance, travel advice, or help choosing the right format first.',
      href: '/contact',
    },
  ]
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF8] text-[#1A1A1A]">
      ...
      <StrategicLinksSection
        eyebrow="Location internal links"
        title={`Plan the next step for your stay in ${config.name}`}
        description={`Use these pages to compare chef-led dining, villa catering, and the fastest conversion paths before you book in ${config.name}.`}
        cards={planningLinks}
      />
    </main>
  )
