...
## 2026-05-17 — Image Dimensions / CLS Stabilization
- **Agent:** Copilot
- **Branch:** auto-improve/image-dimensions-phase4
- **Files:** `src/components/OptimizedImage.tsx`, `src/lib/imageDimensions.ts`, `src/components/{BestPartnerBadge,ConciergeWidget,InVillaServicePage,LocationsHubPage,PremiumPage}.tsx`, `src/components/{catering/CateringPackageCard,events/EventPackageCard,shared/TestimonialBlock}.tsx`, `src/pages/{CateringBBQPage,CateringBabiGulingPage,CateringBuffetPage,CateringCorporatePage,CateringDropOffPage,CateringFloatingBreakfastPage,CateringGrazingPage,CateringMainPage,CateringPlatedPage,CateringRetreatPage,CateringVillaPage,EventsAnniversariesPage,EventsBabyShowersPage,EventsBirthdaysPage,EventsCorporatePage,EventsMainPage,EventsRetreatsPage,EventsVillaPartiesPage,EventsWeddingsPage,PartnerPlatformPage,PartnersPage,ServiceBartendersPage,ServiceButlersPage,ServiceHostPage,ServiceMixologyPage,ServiceSommelierPage,ServiceWaitersPage,StaffingHotelsPage,StaffingHouseholdPage,StaffingLiveInPage,StaffingPlacementPage,StaffingVillaManagersPage,StaffingVillaStaffPage}.tsx`
- **Result:** Added intrinsic image dimensions for every previously incomplete raw `<img>` usage by introducing an `OptimizedImage` wrapper backed by a generated public-asset dimension map, plus fixed the award badge to keep its real 612×408 ratio while remaining responsive. Quality gates: `npx tsc -b --noEmit` ✓, `npm run build` ✓. Raw `<img>` tags missing width/height: 0.
