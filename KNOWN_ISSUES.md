...
3. **CORRECTED — index.html is GTM-only.** Earlier note ("hardcoded GA4 gtag in index.html") was wrong: `G-W0PQH8ZKTF` in `index.html` is only a *comment*. GTM (`GTM-KCBNZBL9`) is the sole loader; GA4 is configured inside GTM. `index.html` NOT changed.
   ...
   - **Could not self-confirm the server-side hit from this browser** (GA4 Realtime showed 0 active users for my session; likely consent-mode/cookieless or a blocker on this profile). Real traffic does flow (28-day stream has data). **Owner to confirm** `generate_lead` populates from WhatsApp clicks in GA4 DebugView/Realtime with live traffic.
