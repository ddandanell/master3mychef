...
2. **Archive `app/` safely — AUDIT DONE 2026-06-23, owner sign-off required (do NOT delete yet).** Stale 1.2 GB duplicate; committed HEAD `48bc7d6` already in prod (no committed work lost); 107 uncommitted files are an abandoned Jun-21/22 state. **Exact backup+delete commands in `OPS_SPRINT_2026-06-23_app-audit-route-policy.md`** (patch/zip the uncommitted diff → verify → `rm -rf app` → rotate token). **`Mychef Live/` is a separate Next.js app — do NOT delete.**
