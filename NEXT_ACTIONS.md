...
- **Next high-value (from Growth Blueprint `implementation_backlog.csv`, reuse don't re-plan):**
  ...
  - **Remaining (lower priority):** `BaliHubPage`/`LocationsHubPage` hub CTAs + per-menu CTAs (TastingMenu ~L396) could pass context into the helper — minor. **Next real value: measurement (watch GA4 `generate_lead` + WhatsApp inbox quality) + pricing-page trust / "how the quote works" clarity + deposit-% consistency (owner).**
