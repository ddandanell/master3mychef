...
3. **Sitemap integrity pass — DONE 2026-06-23: sitemap is clean** (151 URLs, all 200 + self-canonical, 0 redirected/4xx/5xx/blocked/soft-404). _New finding to fix (owner decision):_ ~32 React routes 404 on direct access (not prerendered, no SPA fallback); several are internally linked (sitewide **Footer → `/join-our-team`**, `/villa-chef` ×3, `/recommended-services`). Options: (a) **prerender + sitemap** the genuinely-wanted routes (e.g. `/join-our-team`, `/villa-chef`, `/recommended-services`) so they serve 200 and index — smallest high-value fix; (b) for legacy alias routes (`/terms-of-service`→`/terms`, `/privacy-policy`→`/privacy`, `/payment-terms`→`/cancellation`) add 301 redirects OR repoint the internal links to the canonical URLs; (c) avoid a blanket SPA catch-all rewrite unless soft-404 behaviour is acceptable. Verify each fix returns 200 + correct canonical before/after.
