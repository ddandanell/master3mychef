...
- **Next high-value (from Growth Blueprint `implementation_backlog.csv`, reuse don't re-plan):**
  ...
  - **Remaining CTA opportunities:** make the **global footer + `WhatsAppButton`** route-aware via the same helper; per-menu CTAs (e.g. TastingMenu line ~396) could pass `menu.name` into the helper; extend **sticky mobile CTA** coverage.
