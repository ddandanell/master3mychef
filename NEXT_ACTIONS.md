...
- **Next high-value (from Growth Blueprint `implementation_backlog.csv`, reuse don't re-plan):**
  - TASK-017 **intent-specific CTAs — FIRST GROUP SHIPPED (commit `2b18490`, live).** New reusable helper `src/lib/whatsapp.ts` (`buildWhatsAppUrl`) builds qualified service-matched messages (service+date+guests+area+intent). Applied to 8 top pages: BBQ, villa, buffet catering; bartenders, waiters; weddings, corporate events; `/fine-dining/private-chef-bali`. Verified live (3 page types), canonical number + `data-source` preserved. _Remaining service pages to roll the helper into next: other `/catering/*`, `/in-villa-service/*` (mixology/butlers/host/sommelier), `/events/*`, `/staffing/*`, location pages._
