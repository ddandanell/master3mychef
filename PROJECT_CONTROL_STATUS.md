...
**System learning (do not repeat the mistake):** on Vercel, deployed static route files are generated by `scripts/inject-meta.ts` from **`src/data/sitemap.ts` (`SITEMAP`)** → `dist/<route>/index.html`. `scripts/prerender.ts` is **skipped on Vercel** (no Chromium) and is NOT the production source of truth. To make a route serve 200 on direct access, add it to `src/data/sitemap.ts`. Legacy aliases use `vercel.json` redirects. (See Skill 05 CRITICAL section.)

**Unrelated uncommitted working-tree edits (NOT committed by this sprint — owner to review):** `app/`, `public/_redirects`, `src/data/redirects.tsx`. A `git diff` pager may still be paused in Terminal (safe to `q`/ignore). Repo/worktree commit friction documented in KNOWN_ISSUES #2c → recommended next-sprint priority.
