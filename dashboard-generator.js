#!/usr/bin/env node
/**
 * myCHEF Dashboard Generator
 * 
 * Kører: node dashboard-generator.js
 * Output: dashboard.html (åbn i browser)
 * 
 * Henter data direkte fra mychef.id og genererer visuelt dashboard.
 * Ingen manuelle filer. Ingen vedligeholdelse.
 */
...
function categorizeUrl(url) {
  ...
  if (path === '/') return 'core';
  if (path === '/pricing' || path === '/book' || path === '/partner-platform' || path === '/press') return 'core';
  ...
  // Price pages
  if (path.includes('price') || path.includes('cost') || path.includes('packages')) return 'price';
  
  // Service pages
  if (['chef', 'catering', 'dining', 'event', 'wedding', 'bbq', 'breakfast', 'tasting', 'dinner'].some(k => path.includes(k))) return 'service';
...
function getPriorityScore(url, type) {
  ...
  // Core pages = højeste prioritet
  if (path === '/') return { business: 5, seo: 5, conversion: 5, total: 15 };
  if (path === '/pricing') return { business: 5, seo: 4, conversion: 5, total: 14 };
  if (path === '/book') return { business: 5, seo: 3, conversion: 5, total: 13 };
  
  // Top Bali areas
  const topAreas = ['seminyak', 'canggu', 'ubud', 'uluwatu'];
  if (topAreas.some(a => path.includes(a))) return { business: 5, seo: 5, conversion: 4, total: 14 };
...
<body>
  <div class="container">
    <h1>🔥 myCHEF Dashboard</h1>
    <p class="subtitle">Automatisk genereret ${new Date().toLocaleDateString('da-DK')}</p>
    ...
    <div class="grid">
      <div class="card">
        ...
        <div class="metric">
          ...
          <span class="metric-value status-review">${urls.length}</span>
        </div>
        <div class="metric">
      ...
      <div class="card">
        <h2>⚠️ Kritiske Fejl</h2>
        <div class="metric">
          <span class="metric-label">E002 - Pricing</span>
          <span class="metric-value status-error">David</span>
        </div>
        <div class="metric">
          <span class="metric-label">E003 - Tracking</span>
          <span class="metric-value status-error">Code</span>
        </div>
        <div class="metric">
          <span class="metric-label">E009 - Booking</span>
          <span class="metric-value status-error">Code</span>
        </div>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>${page.score.business}</td>
            <td>${page.score.seo}</td>
            <td>${page.score.conversion}</td>
            <td>
              <div class="score-bar">
            ...
            <td class="status-review">REVIEW</td>
          </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    <div class="footer">
      ...
      <p>Kør <code>node dashboard-generator.js</code> for at opdatere.</p>
    </div>
  </div>
</body>
</html>`;
...
// Main
async function main() {
  console.log('🚀 Genererer myCHEF dashboard...');
