...
- **NON-production / DO NOT EDIT:** `app/` (own `.git`, divergent history, 0 commits in last 5 days) and `Mychef Live/` (own `.git`). See `DO_NOT_TOUCH.md`.
...
4. `find . -maxdepth 3 -name .git -not -path ./.git` → list nested repos (currently `app/`, `Mychef Live/`). Confirm your target file is NOT inside them.
