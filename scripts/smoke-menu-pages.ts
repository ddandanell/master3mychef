...
async function main() {
  ...
  const tcRows = page.locator('button[data-menu-code]')
  check('8 menu rows', (await tcRows.count()) === 8, `got ${await tcRows.count()}`)
  await page.getByRole('button', { name: /Tier A/i }).click()
  check('Tier A filter → 2 rows', (await tcRows.count()) === 2, `got ${await tcRows.count()}`)
  await page.getByRole('button', { name: 'All', exact: true }).click()
  check('All filter → 8 rows', (await tcRows.count()) === 8, `got ${await tcRows.count()}`)
  await tcRows.first().click()
  await page.waitForTimeout(300)
  check('row click → MenuCard visible', (await page.locator('article:visible').count()) >= 1, `got ${await page.locator('article:visible').count()}`)
  check('no image in open card', (await page.locator('article:visible img').count()) === 0, `got ${await page.locator('article:visible img').count()}`)
  const quoteHref = await page.locator('article:visible a[data-source^="three-course-"]').first().getAttribute('href')
  ...
  const kidRows = page.locator('button[data-menu-code]')
  check('6 menu rows', (await kidRows.count()) === 6, `got ${await kidRows.count()}`)
  await kidRows.first().click()
  await page.waitForTimeout(300)
  check('row click → MenuCard visible', (await page.locator('article:visible').count()) >= 1, `got ${await page.locator('article:visible').count()}`)
  check('no image in open card', (await page.locator('article:visible img').count()) === 0)
  ...
  const bbqRows = page.locator('button[data-menu-code]')
  check('12 menu rows', (await bbqRows.count()) === 12, `got ${await bbqRows.count()}`)
  check('3 sub-sections', (await page.locator('h3', { hasText: /Grills$/ }).count()) >= 3)
  await bbqRows.first().click()
  await page.waitForTimeout(300)
  check('row click → MenuCard visible', (await page.locator('article:visible').count()) >= 1, `got ${await page.locator('article:visible').count()}`)
  check('no image in open card', (await page.locator('article:visible img').count()) === 0)
  ...
  check('mobile: rows stack', (await mob.locator('button[data-menu-code]').count()) === 12)
  await mob.locator('button[data-menu-code]').first().click()
  await mob.waitForTimeout(300)
  const box = await mob.locator('article:visible').first().boundingBox()
  check('mobile: open card full-width', !!box && box.width > 340, `width ${box?.width}`)
  await mob.goto(`${BASE}/three-course`, { waitUntil: 'networkidle' })
  await mob.getByRole('button', { name: /Tier C/i }).click()
  check('mobile: filter works', (await mob.locator('button[data-menu-code]').count()) === 3)
