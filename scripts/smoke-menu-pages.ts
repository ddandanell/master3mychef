...
async function main() {
  ...
  check('24 cards on "All"', (await page.locator('#our-menus article').count()) === 24, `got ${await page.locator('#our-menus article').count()}`)
  await page.locator('#our-menus').getByRole('button', { name: 'Seafood' }).click()
  check('Seafood filter → 6', (await page.locator('#our-menus article').count()) === 6)
  ...
  check('24 cards', (await page.locator('article:has(a[data-source^="finedining-menus-"])').count()) === 24, `got ${await page.locator('article:has(a[data-source^="finedining-menus-"])').count()}`)
